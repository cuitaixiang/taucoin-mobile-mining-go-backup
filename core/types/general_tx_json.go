// Code generated by github.com/fjl/gencodec. DO NOT EDIT.

package types

import (
	"encoding/json"
	"errors"
	"math/big"

	"github.com/Tau-Coin/taucoin-mobile-mining-go/common"
	"github.com/Tau-Coin/taucoin-mobile-mining-go/common/hexutil"
)

var _ = (*GeneralTxMarshaling)(nil)

// MarshalJSON marshals as JSON.
func (g GeneralTx) MarshalJSON() ([]byte, error) {
	type GeneralTx struct {
		Version   hexutil.Bytes   `json:"version"     gencodec:"required"`
		Option    hexutil.Bytes   `json:"option"      gencodec:"required"`
		ChainID   hexutil.Bytes   `json:"chainid"     gencodec:"required"`
		Nounce    hexutil.Uint64  `json:"nounce"      gencodec:"required"`
		TimeStamp hexutil.Uint32  `json:"timestamp"   gencodec:"required"`
		Fee       hexutil.Bytes   `json:"fee"         gencodec:"required"`
		V         *hexutil.Big    `json:"v"           gencodec:"required"`
		R         *hexutil.Big    `json:"r"           gencodec:"required"`
		S         *hexutil.Big    `json:"s"           gencodec:"required"`
		Sender    *common.Address `json:"sender"        rlp:"required"`
	}
	var enc GeneralTx
	enc.Version = hexutil.Bytes(g.Version)
	enc.Option = hexutil.Bytes(g.Option)
	enc.ChainID = hexutil.Bytes(g.ChainID)
	enc.Nounce = hexutil.Uint64(g.Nounce)
	enc.TimeStamp = hexutil.Uint32(g.TimeStamp)
	enc.Fee = hexutil.Bytes(g.Fee)
	enc.V = (*hexutil.Big)(g.V)
	enc.R = (*hexutil.Big)(g.R)
	enc.S = (*hexutil.Big)(g.S)
	enc.Sender = g.Sender
	return json.Marshal(&enc)
}

// UnmarshalJSON unmarshals from JSON.
func (g *GeneralTx) UnmarshalJSON(input []byte) error {
	type GeneralTx struct {
		Version   *hexutil.Bytes  `json:"version"     gencodec:"required"`
		Option    *hexutil.Bytes  `json:"option"      gencodec:"required"`
		ChainID   *hexutil.Bytes  `json:"chainid"     gencodec:"required"`
		Nounce    *hexutil.Uint64 `json:"nounce"      gencodec:"required"`
		TimeStamp *hexutil.Uint32 `json:"timestamp"   gencodec:"required"`
		Fee       *hexutil.Bytes  `json:"fee"         gencodec:"required"`
		V         *hexutil.Big    `json:"v"           gencodec:"required"`
		R         *hexutil.Big    `json:"r"           gencodec:"required"`
		S         *hexutil.Big    `json:"s"           gencodec:"required"`
		Sender    *common.Address `json:"sender"        rlp:"required"`
	}
	var dec GeneralTx
	if err := json.Unmarshal(input, &dec); err != nil {
		return err
	}
	if dec.Version == nil {
		return errors.New("missing required field 'version' for GeneralTx")
	}
	g.Version = OneByte(*dec.Version)
	if dec.Option == nil {
		return errors.New("missing required field 'option' for GeneralTx")
	}
	g.Option = OneByte(*dec.Option)
	if dec.ChainID == nil {
		return errors.New("missing required field 'chainid' for GeneralTx")
	}
	g.ChainID = Byte32s(*dec.ChainID)
	if dec.Nounce == nil {
		return errors.New("missing required field 'nounce' for GeneralTx")
	}
	g.Nounce = uint64(*dec.Nounce)
	if dec.TimeStamp == nil {
		return errors.New("missing required field 'timestamp' for GeneralTx")
	}
	g.TimeStamp = uint32(*dec.TimeStamp)
	if dec.Fee == nil {
		return errors.New("missing required field 'fee' for GeneralTx")
	}
	g.Fee = OneByte(*dec.Fee)
	if dec.V == nil {
		return errors.New("missing required field 'v' for GeneralTx")
	}
	g.V = (*big.Int)(dec.V)
	if dec.R == nil {
		return errors.New("missing required field 'r' for GeneralTx")
	}
	g.R = (*big.Int)(dec.R)
	if dec.S == nil {
		return errors.New("missing required field 's' for GeneralTx")
	}
	g.S = (*big.Int)(dec.S)
	if dec.Sender != nil {
		g.Sender = dec.Sender
	}
	return nil
}
